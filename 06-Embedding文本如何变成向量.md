...
前面知道了模型会把文本拆成 token，但 token 还只是离散编号。继续往下看，真正关键的问题会变成：这些离散编号怎样被组织成能够表达语义关系的向量？
...
这里是 Embedding 的补充理解层。  
如果只对标这一节课程，先把前面的“token -> 向量表 -> 上下文编码 -> 句向量”吃透就够了；这一段是为了让你知道，模型为什么会真的学成这样。
