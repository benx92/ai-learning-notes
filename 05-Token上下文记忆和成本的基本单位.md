...
- `$输入Token数$` 包括系统规则、用户问题、历史对话、文档片段、工具定义等。
- `$输出Token数$` 是模型生成回答时消耗的 Token。
- `$总Token数$` 必须落在模型支持的上下文窗口内。
...
$$
第n轮输入Token数 \approx 系统规则 + 历史消息Token数 + 当前用户消息Token数
$$
...
- `$输入Token数$` 是本次请求送给模型看的内容。
- `$输出Token数$` 是模型生成的内容。
- `$输入单价$` 和 `$输出单价$` 通常按每 100 万 Token 计价。
